...
export default function Sidebar() {
  ...
  // ✅ FIX 1: Auto-open sub-menu based on current route
  // ✅ FIX 2: Close sub-menu when navigating to a non-sub-item route
  useEffect(() => {
    const activeParent = findActiveParent();
    if (activeParent) {
      // Current route belongs to a sub-menu → keep it open
      setOpenMenu(activeParent.path);
    } else {
      // Current route is a standalone tab → close any open sub-menu
      setOpenMenu(null);
    }
  }, [location.pathname]);

  // ✅ FIX 3: When sidebar expands, re-open the sub-menu if a sub-item is active
  useEffect(() => {
    if (!isCollapsed) {
      const activeParent = findActiveParent();
      if (activeParent) {
  ...
  const isSubItemActive = (subItem, parentPath) => {
    if (subItem.path === parentPath) {
  ...
  const handleMenuClick = (item, e) => {
    if (item.subItems) {
      // ✅ Parent with sub-items: toggle sub-menu, don't navigate
      e.preventDefault();
      ...
      // ✅ FIX 1: Standalone tab clicked → close any open sub-menu
      setOpenMenu(null);
    }
  };
                              ...
                              return (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    {/* {isSubActive && (
                                      <div className="absolute right-[0.4vw] top-1/2 -translate-y-1/2 w-[0.35vw] h-[0.35vw] rounded-full bg-blue-500 animate-pulse" />
                                    )} */}
                                  </Link>
                                </li>
                              );
