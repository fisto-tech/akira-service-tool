...
const ReportsModal = ({ row, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-[2vw]">
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-[70vw] max-h-[90vh] rounded-[0.8vw] shadow-2xl overflow-hidden flex flex-col border border-gray-300">
        ...
        <div className="flex-1 overflow-y-auto p-[1.5vw] space-y-[1vw]">
          <div className="grid grid-cols-3 gap-[1vw] mb-[1vw]">
            <div className="bg-slate-50 rounded-[0.6vw] p-[1vw] border border-gray-200">
              <div className="text-[0.7vw] uppercase  font-semibold text-gray-600 mb-[0.6vw]">Entry Details</div>
              <div className="space-y-[0.45vw]"><div className="text-[0.78vw] text-gray-800"><span className="font-semibold">Date:</span> {fmtDate(row.date)}</div><div className="text-[0.78vw] text-gray-800"><span className="font-semibold">Job Order:</span> {row.jobOrderNo || "—"}</div><div className="text-[0.78vw] text-gray-800"><span className="font-semibold">Products:</span> {products.length}</div></div>
            </div>
            <div className="bg-slate-50 rounded-[0.6vw] p-[1vw] border border-gray-200">
              <div className="text-[0.7vw] uppercase  font-semibold text-gray-600 mb-[0.6vw]">Customer</div>
              <div className="text-[0.78vw] text-gray-800"><span className="font-semibold">Name:</span> {products[0]?.customerName || "—"}</div>
              <div className="text-[0.78vw] text-gray-800"><span className="font-semibold">Code:</span> {products[0]?.customerCode || "—"}</div>
            </div>
            <div className="bg-slate-50 rounded-[0.6vw] p-[1vw] border border-gray-200">
              <div className="text-[0.7vw] uppercase  font-semibold text-gray-600 mb-[0.6vw]">Report Summary</div>
              <div className="text-[0.78vw] text-gray-800"><span className="font-semibold">Products:</span> {products.length}</div>
              <div className="text-[0.78vw] text-gray-800"><span className="font-semibold">Last updated:</span> {fmtDate(row.updatedAt || row.createdAt)}</div>
            </div>
          </div>

          {products.length === 0 ? (
            <div className="text-center py-[4vw] text-[0.9vw] text-gray-500">No product records available for this entry.</div>
          ) : products.map((prod, idx) => (
            <div key={prod._pid} className="border border-gray-200 rounded-[0.6vw] overflow-hidden shadow-sm">
              <div className="bg-blue-50 px-[1.2vw] py-[1vw] border-b border-blue-100 flex items-center justify-between gap-[1vw]">
                <div>
                  <div className="text-[0.95vw] font-bold text-blue-900">{prod.productDescription || "Unnamed Product"}</div>
                  <div className="text-[0.72vw] text-gray-600 mt-[0.3vw] flex flex-wrap gap-[0.8vw]"><span>JO: {prod.jobOrderNo || "—"}</span><span>Code: {prod.productCode || "—"}</span><span>Qty: {prod.quantity || "—"}</span></div>
                </div>
                <div className="text-[0.72vw] text-gray-700 uppercase  font-semibold">Item {idx + 1}</div>
              </div>

              <div className="grid grid-cols-4 gap-[1vw] p-[1.2vw] bg-white border-b border-gray-200 text-[0.8vw] text-gray-800">
                <div><div className="text-gray-500 mb-[0.35vw] uppercase  text-[0.65vw]">Stage</div><div className="font-semibold">{prod.stage || "—"}</div></div>
                <div><div className="text-gray-500 mb-[0.35vw] uppercase  text-[0.65vw]">Serial / Batch</div><div className="font-semibold">{prod.identification || "—"}</div></div>
                <div><div className="text-gray-500 mb-[0.35vw] uppercase  text-[0.65vw]">Delay Time</div><div className="font-semibold">{prod.delayTime || "—"}</div></div>
                <div><div className="text-gray-500 mb-[0.35vw] uppercase  text-[0.65vw]">Category</div><div className="font-semibold">{prod.category || "—"}</div></div>
              </div>

              {/* Added: Item Level Detailed Info from Form */}
              <div className="p-[1.2vw] space-y-[1.2vw] bg-white border-b border-gray-100">
                <div className="grid grid-cols-2 gap-[1vw]">
                  <div className="bg-slate-50 p-[0.8vw] rounded-[0.5vw] border border-slate-100">
                    <div className="text-[0.65vw] text-blue-600 font-bold uppercase mb-[0.5vw] ">Problem Description</div>
                    <div className="text-[0.8vw] text-gray-800 font-medium leading-relaxed">{prod.problem || "No description provided."}</div>
                  </div>
                  <div className="bg-slate-50 p-[0.8vw] rounded-[0.5vw] border border-slate-100">
                    <div className="text-[0.65vw] text-blue-600 font-bold uppercase mb-[0.5vw] ">Possible Root Cause(s)</div>
                    <div className="text-[0.8vw] text-gray-800 font-medium leading-relaxed">{prod.rootCause || "No root cause identified."}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-[1vw]">
                  <div className="bg-slate-50 p-[0.8vw] rounded-[0.5vw] border border-slate-100">
                    ...
                    {parseParts(prod.partsDetails).length > 0 ? (
                      <ul className="text-[0.8vw] text-gray-800 space-y-[0.3vw] list-disc list-inside font-medium">
                        {parseParts(prod.partsDetails).map((part, i) => (
                          <li key={i}>{part}</li>
                        ))}
                      </ul>
                    ) : (
                      <div className="text-[0.8vw] text-gray-500 italic">No parts details recorded.</div>
                    )}
                  </div>
                  <div className="bg-slate-50 p-[0.8vw] rounded-[0.5vw] border border-slate-100">
                    <div className="text-[0.65vw] text-blue-600 font-bold uppercase mb-[0.5vw] ">Corrective Action</div>
                    <div className="text-[0.8vw] text-gray-800 font-medium leading-relaxed">{prod.correction || "No corrective action recorded."}</div>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-[1vw]">
                  <div className="bg-slate-50 p-[0.7vw] rounded-[0.5vw] border border-slate-100 text-center">
                    <div className="text-[0.65vw] text-gray-400 font-bold uppercase mb-[0.2vw]">Disposition</div>
                    <div className="text-[0.8vw] font-bold text-gray-900">{prod.disposition || "—"}</div>
                  </div>
                  <div className="bg-slate-50 p-[0.7vw] rounded-[0.5vw] border border-slate-100 text-center">
                    <div className="text-[0.65vw] text-gray-400 font-bold uppercase mb-[0.2vw]">Completed Date</div>
                    <div className="text-[0.8vw] font-bold text-gray-900">{fmtDate(prod.finalStatusDate) || "—"}</div>
                  </div>
                  <div className="bg-blue-50 p-[0.7vw] rounded-[0.5vw] border border-blue-100 text-center">
                    <div className="text-[0.65vw] text-blue-400 font-bold uppercase mb-[0.2vw]">Final Status</div>
                    <div className="text-[0.8vw] font-black text-blue-700">{prod.finalStatus || "Pending"}</div>
                  </div>
                </div>
              </div>

              <div className="p-[1.2vw] bg-gray-50/50">
                <div className="text-[0.7vw] font-black text-gray-400 uppercase tracking-[0.2em] mb-[1vw] flex items-center gap-[0.5vw]">
                ...
                <div className="grid grid-cols-3 gap-[1vw]">
                  {['assembledBy', 'testedBy', 'fiBy'].map(role => {
                    ...
                    return (
                      <div key={role} className="bg-white border border-gray-200 rounded-[0.5vw] p-[0.8vw] shadow-sm flex flex-col gap-[0.6vw]">
                        <div className="flex items-center gap-[0.8vw]">
                          <div className="w-[2vw] h-[2vw] rounded-full bg-blue-50 flex items-center justify-center shrink-0">
                            <User className="w-[1vw] h-[1vw] text-blue-500" />
                          </div>
                          <div className="min-w-0">
                            <div className="text-[0.6vw] text-gray-600 font-bold uppercase ">{label}</div>
                            <div className="text-[0.8vw] font-bold text-gray-900 truncate">{name}</div>
                          </div>
                        </div>
                        {res && (
                          <div className="bg-blue-50/50 p-[0.6vw] rounded-[0.4vw] border border-blue-100 flex flex-col gap-[0.4vw]">
                            {res.problem && (
                              <div>
                                <div className="text-[0.55vw] font-bold text-blue-600 uppercase tracking-tighter">Employee Feedback</div>
                                <div className="text-[0.68vw] text-blue-900 font-medium">{res.problem}</div>
                              </div>
                            )}
                            {parseParts(res.partsDetails).length > 0 && (
                              <div>
                                <div className="text-[0.55vw] font-bold text-blue-600 uppercase tracking-tighter">Parts Replacement</div>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-visible">
            <div className="border-t border-gray-300 px-[1.2vw] pt-[1.2vw] pb-[1.5vw] space-y-[1.2vw]">
              ...
              <div className="grid grid-cols-3 gap-[1vw] mt-[1vw]">
                ...
                <div className="flex flex-col gap-[0.4vw]">
                  <label className="text-[0.72vw] font-semibold text-black">Delay Time</label>
                  <input value={prod.delayTime} onChange={e => sp("delayTime", e.target.value)} placeholder="e.g. 2 days" disabled={isReadOnly} className="border border-gray-300 rounded-[0.4vw] py-[0.55vw] px-[0.6vw] text-[0.78vw] outline-none focus:border-blue-400" />
                </div>
                <div className="flex flex-col gap-[0.4vw]">
      ...
      <div className="w-full h-full font-sans text-[0.85vw]">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
          ...
          <div className="flex gap-[1vw] mb-[0.9vw] items-center flex-wrap">
            <div className="flex bg-gray-100 p-[0.3vw] rounded-[0.6vw] border border-gray-200">
              {['All', 'Admin', 'Employee'].map(f => (
                <button
                  key={f}
                  onClick={() => setSourceFilter(f)}
                  className={`px-[1vw] py-[0.4vw] rounded-[0.4vw] text-[0.75vw] font-bold transition-all ${sourceFilter === f ? "bg-white text-blue-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                >
                  {f === 'Admin' ? "Admin's" : f === 'Employee' ? "Employee's" : "All Entries"}
                </button>
              ))}
            </div>
            <div className="h-[1.5vw] w-px bg-gray-300 mx-[0.2vw]" />
            ...
            <div className="flex items-center gap-[0.4vw] bg-white border border-gray-300 rounded-[0.5vw] px-[0.6vw] py-[0.35vw]">
              <User className="w-[0.9vw] h-[0.9vw] text-gray-400" />
              <select 
                value={employeeFilter} 
                onChange={e => setEmployeeFilter(e.target.value)}
                className="text-[0.75vw] font-bold text-gray-700 border-none outline-none bg-transparent cursor-pointer"
              >
            ...
            <div className="overflow-auto max-h-[65vh] min-h-[65vh] w-full rounded-t-[0.6vw]">
              <table className="w-full text-left border-collapse">
                <thead className="bg-blue-50 sticky top-0 z-10">
                  ...
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Customer</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Cus Code</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Item Code</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Item Description</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Category</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Stage</th>
                  ...
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Serial/Batch</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Assembled</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Tested</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">FI By</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Delay Time</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Reports</th>
                  ...
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Employee Name</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-r border-gray-300 whitespace-nowrap text-[0.82vw] align-middle text-center">Final Remarks</th>
                  <th className="px-[0.8vw] py-[0.8vw] font-semibold text-gray-900 border-b-2 border-blue-400 whitespace-nowrap text-[0.82vw] align-middle text-center">Action</th>
                </tr>
              </thead>
              <tbody>
                      ...
                      return (
                        <tr key={`${row.id}-${pi}`} className={`transition-colors hover:bg-gray-50/60 ${bgCls} ${pi > 0 ? "border-t border-dashed border-gray-300" : "border-t border-gray-300"}`}>
                          ...
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 align-middle text-center">
                            <div className="text-[0.75vw] font-semibold text-gray-800 truncate max-w-[10vw] mx-auto text-center" title={prod.customerName}>{prod.customerName || "—"}</div>
                          </td>
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 text-center align-middle">
                            <div className="text-[0.72vw] font-medium text-gray-900 mx-auto text-center">{prod.customerCode || "—"}</div>
                          </td>
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 text-center align-middle">
                          ...
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 text-center align-middle">
                            <div className="text-[0.68vw] font-semibold text-blue-600 px-[0.4vw] py-[0.1vw] rounded mx-auto text-center w-max">{prod.category || "—"}</div>
                          </td>
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 text-center align-middle">
                          ...
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 text-center align-middle font-bold text-black text-[0.75vw]">{prod.quantity}</td>
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 align-middle text-center">
                          ...
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 align-middle text-center">
                            <div className="text-[0.72vw] text-gray-800 font-medium truncate max-w-[7vw] mx-auto" title={prod.delayTime || "—"}>{prod.delayTime || "—"}</div>
                          </td>

                          {pi === 0 && (
                            <td rowSpan={span} className="px-[0.8vw] py-[0.7vw] text-center border-r border-gray-300 align-middle">
                          ...
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 align-middle text-center">
                            <div className="flex flex-col items-center">
                              <div className="text-[0.75vw] px-[0.4vw] py-[0.1vw] bg-amber-50 text-amber-700 border border-amber-200 rounded-full font-bold">
                                {row.creatorName || "Admin"}
                              </div>
                            </div>
                          </td>
                          <td className="px-[0.8vw] py-[0.7vw] border-r border-gray-300 align-middle text-center min-w-[10vw]">
                            <div className="text-[0.72vw] text-gray-800 font-medium line-clamp-2 leading-tight italic mx-auto text-center" title={latestRemark}>
                              {latestRemark}
                            </div>
                          </td>

                          {pi === 0 && (
                            <td rowSpan={span} className="px-[0.8vw] py-[0.7vw] text-center align-middle">
