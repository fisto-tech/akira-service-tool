...
const TestingRepairReport = ({ entries }) => {
  ...
  const exportCsv = () => {
    const headers = ["S.No","Date","Customer","Ref (Customer)","Product","Serial No","Type","Tested By","Completed Date","Problem Identified","Root Cause","Corrective Action","Parts Replaced","Status"];
    const data = filtered.map(({ row, p }, i) => [
      i+1, fmtDate(row.date), row.customerName, row.refNoCustomer,
      p.productDescription, p.serialNumber, p.type === "W" ? "Warranty" : "Paid",
  ...
  return (
    <div>
      ...
      <div className="overflow-auto rounded-[0.5vw] border border-slate-300">
        <table id="table-testing" className="w-full border-collapse text-left">
          <thead className="sticky top-0 z-10">
            <tr>
              <Th>S.No</Th><Th cls="min-w-[6vw]">Date</Th><Th>Customer</Th><Th>Ref No.</Th>
              <Th>Product</Th><Th>Serial No</Th><Th>Type</Th>
              <Th>Tested By</Th><Th>Completed Date</Th>
            ...
            {filtered.length === 0 ? <EmptyState message="No testing/repair reports found." /> :
              filtered.map(({ row, p }, i) => (
                <tr key={`${row.id}-${p._pid}`} className="hover:bg-gray-50/60">
                  ...
                  <Td cls="max-w-[10vw]"><div className="truncate" title={p.productDescription}>{p.productDescription || "—"}</div></Td>
                  ...
                  <Td cls="max-w-[12vw]"><div className="line-clamp-2 text-[0.68vw]">{p.report?.problemDescription || "—"}</div></Td>
                  <Td cls="max-w-[12vw]"><div className="line-clamp-2 text-[0.68vw]">{p.report?.rootCause || "—"}</div></Td>
                  <Td cls="max-w-[12vw]"><div className="line-clamp-2 text-[0.68vw]">{p.report?.correctiveAction || "—"}</div></Td>
...
const DeliveryReport = ({ entries }) => {
  ...
  return (
    <div>
      ...
      <div className="overflow-auto rounded-[0.5vw] border border-slate-300">
        <table id="table-delivery" className="w-full border-collapse text-left">
          <thead className="sticky top-0 z-10">
            <tr>
            ...
            {filtered.length === 0 ? <EmptyState message="No delivery records found." /> :
              filtered.map(({ row, prods, tat, avgDelay }, i) => (
                <tr key={row.id} className="hover:bg-gray-50/60">
                  <Td cls="text-center">{i+1}</Td>
                  <Td cls="min-w-[6vw]">{fmtDate(row.date)}</Td>
                  <Td cls="font-semibold max-w-[9vw]"><div className="truncate" title={row.customerName}>{row.customerName || "—"}</div></Td>
                  <Td>{row.refNoCustomer || "—"}</Td>
                  <Td>{row.category || "—"}</Td>
                  ...
                  <Td cls="max-w-[10vw]"><div className="truncate text-[0.68vw]" title={row.finalStatusRemarks}>{row.finalStatusRemarks || "—"}</div></Td>
                </tr>
              ))
            }
          </tbody>
...
const RootCauseReport = ({ entries }) => {
  ...
  const exportCsv = () => {
    const headers = ["S.No","Date","Customer","Ref No.","Product","Serial No","Type","Problem Identified","Root Cause","Contributing Factors","Tested By","Report Status"];
    const data = filtered.map(({ row, p }, i) => [
      i+1, fmtDate(row.date), row.customerName, row.refNoCustomer,
      p.productDescription, p.serialNumber, p.type === "W" ? "Warranty" : "Paid",
  ...
  return (
    <div>
      ...
      <div className="overflow-auto rounded-[0.5vw] border border-slate-300">
        <table id="table-rca" className="w-full border-collapse text-left">
          <thead className="sticky top-0 z-10">
            <tr>
              ...
              filtered.map(({ row, p }, i) => (
                <>
                  <tr key={`${row.id}-${p._pid}`} className="hover:bg-amber-50/30">
                    <Td cls="text-center">{i+1}</Td>
                    <Td cls="min-w-[6vw]">{fmtDate(row.date)}</Td>
                    <Td cls="font-semibold max-w-[8vw]"><div className="truncate" title={row.customerName}>{row.customerName || "—"}</div></Td>
                    <Td cls="max-w-[10vw]"><div className="truncate" title={p.productDescription}>{p.productDescription || "—"}</div></Td>
                    <Td>{p.serialNumber || "—"}</Td>
                    <Td>
                      <span className={`px-[0.4vw] py-[0.05vw] rounded text-[0.62vw] font-bold ${p.type === "W" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"}`}>
                    ...
                    <Td cls="max-w-[11vw]"><div className="line-clamp-2 text-[0.68vw]">{p.report?.problemDescription || "—"}</div></Td>
                    <Td cls="max-w-[11vw]"><div className="line-clamp-2 text-[0.68vw] font-semibold text-amber-800">{p.report?.rootCause || "—"}</div></Td>
                    <Td>{p.report?.testedBy || "—"}</Td>
                    <Td><StatusBadge status={p.report?.status} /></Td>
                    <Td cls="text-center">
                      <button type="button"
...
const CorrectiveActionReport = ({ entries }) => {
  ...
  const exportCsv = () => {
    const headers = ["S.No","Date","Customer","Ref No.","Product","Serial No","Type","Root Cause","Corrective Action Taken","Parts Replaced","Implemented By","Completed Date","Verification Status"];
    const data = filtered.map(({ row, p }, i) => [
      i+1, fmtDate(row.date), row.customerName, row.refNoCustomer,
      p.productDescription, p.serialNumber, p.type === "W" ? "Warranty" : "Paid",
  ...
  return (
    <div>
      ...
      <div className="overflow-auto rounded-[0.5vw] border border-slate-300">
        <table id="table-corrective" className="w-full border-collapse text-left">
          <thead className="sticky top-0 z-10">
            <tr>
              <Th>S.No</Th><Th cls="min-w-[6vw]">Date</Th><Th>Customer</Th>
              <Th>Product</Th><Th>Serial No</Th><Th>Type</Th>
              ...
              <Th>Completed Date</Th><Th>Verification</Th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? <EmptyState message="No corrective action records found." /> :
              filtered.map(({ row, p }, i) => (
                <tr key={`${row.id}-${p._pid}`} className="hover:bg-teal-50/20">
                  ...
                  <Td cls="max-w-[10vw]"><div className="line-clamp-2 text-[0.68vw] text-amber-800">{p.report?.rootCause || "—"}</div></Td>
                  <Td cls="max-w-[12vw]"><div className="line-clamp-2 text-[0.68vw] font-semibold text-teal-800">{p.report?.correctiveAction || "—"}</div></Td>
                  <Td>{p.report?.partsReplacement || "—"}</Td>
                  <Td>
