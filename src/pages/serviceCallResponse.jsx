...
const SupportReqActionPanel = ({ req, currentUser, onDone }) => {
  ...
  const handleClose = () => {
    ...
    onDone({ status: "Resolved", resolutionNotes: remarks, resolvedAt: new Date().toISOString(), resolutionType });
    const q = loadQ();
    const upd = q.map(e => {
      if (e.callId !== req.callId) return e;
      const products = (e.products || []).map((p, i) => i === req.productIdx ? { ...p, _resolved: true, _productClosure: { status: "Closed", resolutionType, remarks, closedAt: new Date().toISOString() } } : p);
      return { ...e, products };
    });
    saveQ(upd);
  };
...
const SupportRequestsTab = ({ currentUser }) => {
  ...
  useEffect(() => {
    const l = () => setReqs(load(SUPPORT_REQ_KEY, []).filter(r => r.supportPerson?.userId === currentUser?.userId && r.status !== "Resolved"));
    l(); const i = setInterval(l, 2000); return () => clearInterval(i);
  }, [currentUser]);
  ...
  return (
    <div className="space-y-[0.8vw]">
      <div className="bg-white rounded-[0.6vw] p-[1vw] border border-gray-200 flex items-center justify-between">
        ...
        <Badge label={`${reqs.length} Active`} color="orange" />
      </div>
      {reqs.length === 0 ? <div className="bg-white rounded-[0.5vw] p-[3vw] text-center border border-gray-100 text-gray-400">No open support requests at this time.</div> : (
        <div className="grid gap-[0.8vw]">
          {reqs.map(r => (
            <div key={r.id} className="bg-white rounded-[0.5vw] border border-gray-200 p-[0.8vw] shadow-sm flex flex-col gap-[0.5vw] transition-all hover:border-blue-300">
               <div className="flex justify-between items-start">
                 <div>
               ...
               <SupportReqActionPanel req={r} currentUser={currentUser} onDone={(d) => { const all = load(SUPPORT_REQ_KEY, []); save(SUPPORT_REQ_KEY, all.map(x => x.id === r.id ? { ...x, ...d, status: "Resolved" } : x)); }} />
            </div>
          ))}
        </div>
      )}
...
const VisitsTab = ({ type, currentUser }) => {
  const [visits, setVisits] = useState([]);
  useEffect(() => {
    const l = () => setVisits(load(FIELD_VISIT_KEY, []).filter(v => v.assignedTo === currentUser?.userId && v.type === type && v.visitStatus !== "Closed"));
    l(); const i = setInterval(l, 2000); return () => clearInterval(i);
  }, [currentUser, type]);
  ...
  return (
    <div className="space-y-[0.8vw]">
      <div className="bg-white rounded-[0.6vw] p-[1vw] border border-gray-200 flex items-center justify-between">
        ...
        <Badge label={`${visits.length} Scheduled`} color="blue" />
      </div>
      {visits.length === 0 ? <div className="bg-white rounded-[0.5vw] p-[3vw] text-center border border-gray-100 text-gray-500">No upcoming field visits.</div> : (
        <div className="grid gap-[0.8vw]">
          {visits.map(v => (
            <div key={v.id} className="bg-white rounded-[0.5vw] border border-gray-200 p-[0.8vw] shadow-sm flex flex-col gap-[0.6vw] hover:border-blue-300 transition-all">
              <div className="flex justify-between items-start">
                <span className="text-[0.95vw] font-bold text-gray-800">{v.callNumber}</span>
                <Badge label={`Scheduled: ${v.visitDate}`} color="blue" />
              </div>
              ...
              <button 
                onClick={() => { const all = load(FIELD_VISIT_KEY, []); save(FIELD_VISIT_KEY, all.map(x => x.id === v.id ? { ...x, visitStatus: "Closed", closedAt: new Date().toISOString() } : x)); }} 
...
const ReportsTab = ({ currentUser }) => {
  ...
  useEffect(() => {
    const fetchReports = () => {
      ...
      const reqs = load(SUPPORT_REQ_KEY, []).filter(r => r.supportPerson?.userId === uid && r.status === "Resolved");
      reqs.forEach(r => {
        acts.push({
          id: `sup_${r.id}`,
      ...
      const visits = load(FIELD_VISIT_KEY, []).filter(v => v.assignedTo === uid && v.visitStatus === "Closed");
      visits.forEach(v => {
        acts.push({
          id: `vis_${v.id}`,
          type: "Field Visit Completed",
          title: `Call ${v.callNumber} • Scheduled ${v.visitDate}`,
...
const ServiceCallResponse = () => {
  ...
  const liveCounts = useMemo(() => {
    ...
    return {
      ...
      support:    load(SUPPORT_REQ_KEY, []).filter(r => r.supportPerson?.userId === uid && r.status !== "Resolved").length,
      fieldVisit: load(FIELD_VISIT_KEY, []).filter(r => r.assignedTo === uid && r.visitStatus !== "Closed").length,
  ...
  const handleProductClose = (callId, pIdx, d) => {
    const upd = loadQ().map(e => {
      if (e.callId !== callId) return e;
      const products = (e.products || []).map((p, i) => i === pIdx ? { ...p, _productClosure: d } : p);
      const allDone  = products.every(p => p._resolved || p._productClosure?.status === "Closed");
  ...
  const handleSupportRequest = (callId, pIdx, data) => {
    ...
    reqs.push({ id: Date.now(), ...data, callId, callNumber: entry.callNumber, productIdx: pIdx, product: entry.products[pIdx], status: "Pending", createdAt: new Date().toISOString() });
    save(SUPPORT_REQ_KEY, reqs);
    const upd = q.map(e => {
       if (e.callId !== callId) return e;
       const products = (e.products || []).map((p, i) => i === pIdx ? { ...p, _supportRequested: true, _supportPersonName: data.supportPerson.name } : p);
       return { ...e, products, status: "Escalated" };
    });
    saveQ(upd); setQueue(upd);
  };

  const handleAssignVisit = (callId, type, form, productIdx) => {
    const entry = loadQ().find(e => e.callId === callId); if (!entry) return;
    const records = load(FIELD_VISIT_KEY, []);
    records.push({ id: Date.now(), ...form, callId, callNumber: entry.callNumber, productIdx, visitStatus: "Open", type, createdAt: new Date().toISOString() });
  ...
  return (
    <div className="flex flex-col h-full text-[0.85vw]">
      ...
      <div className="flex-1 overflow-y-auto pr-[0.3vw] pb-[5vw]">
        ...
        {activeTab === "fieldvisit" && <VisitsTab type="Field Visit" currentUser={loggedInUser} />}
