...
const COLUMN_VIS_KEY = "customer_db_col_visibility_v1";
...
function loadColVisibility() {
  try {
    const stored = JSON.parse(localStorage.getItem(COLUMN_VIS_KEY) || "{}");
...
const CustomerDatabase = () => {
  ...
  useEffect(() => {
    const loadPartyTypes = () => {
      try {
        const s = localStorage.getItem(PARTY_TYPES_KEY);
        if (s) {
    ...
    setPartyTypes(loadPartyTypes());
    const stored = localStorage.getItem(STORAGE_KEY);
    if (stored) {
      try {
        const p = JSON.parse(stored);
        setData(p.sort((a, b) => a.partyCode.localeCompare(b.partyCode)));
      } catch {}
    }
  }, []);
  ...
  const handleManualSubmit = (e) => {
    ...
    // Separate existing items (being updated) vs new items
    const existingItems = newEntry.items.filter((it) => it._existing);
    const newItems = newEntry.items.filter((it) => !it._existing);

    // Validate new items for duplicate item codes
    const dups = newItems.filter((ni) =>
      data.some(
        (d) =>
          cleanStr(d.itemCode) === cleanStr(ni.itemCode) &&
          // allow if it was already linked to this party (shouldn't happen for new, but safety)
          d.partyCode !== newEntry.partyCode,
      ),
    );
    if (dups.length > 0) {
      toast(`Item Code "${dups[0].itemCode}" already exists in another party!`, "error");
      return;
    }

    // Also check new items against existing items of same party
    const existingCodes = new Set(existingItems.map((it) => cleanStr(it.itemCode)));
    const dupInNew = newItems.filter((ni) => ni.itemCode && existingCodes.has(cleanStr(ni.itemCode)));
    if (dupInNew.length > 0) {
      toast(`Item Code "${dupInNew[0].itemCode}" already exists for this party!`, "error");
      return;
    }

    // Validate warranty for new items only (existing items validated in edit flow)
    const invalidWarranty = newItems.find((item) => {
      if (!item.itemCode) return false; // skip blank rows
      const val = Number(item.warrantyPeriodDays);
      return item.warrantyPeriodDays === "" || isNaN(val) || val < 0;
    });
    if (invalidWarranty) {
      toast(`Invalid Warranty for Item "${invalidWarranty.itemCode || "Unknown"}". Must be a non-negative number.`, "error");
      return;
    }

    // Filter out completely blank new item rows
    const validNewItems = newItems.filter((item) => item.itemCode && item.itemCode.trim());

    if (validNewItems.length === 0 && existingItems.length === 0) {
      toast("Please add at least one item.", "error");
      return;
    }

    // Remove all rows for this party, then re-add existing (possibly edited) + new
    const partyCode = newEntry.partyCode;
    const otherData = data.filter((d) => d.partyCode !== partyCode);

    // Re-build existing rows with updated party-level info
    ...
    saveToStorage([...otherData, ...updatedExistingRows, ...newRows]);
  ...
  const handleEditSubmit = (e) => {
    ...
    // Validate warranty for all items
    const invalidWarranty = editEntry.items.find((item) => {
      const val = Number(item.warrantyPeriodDays);
      return item.warrantyPeriodDays === "" || isNaN(val) || val < 0;
    });
    if (invalidWarranty) {
    ...
    // Check for duplicate item codes within this party
    const codes = editEntry.items.map((it) => cleanStr(it.itemCode));
    const hasDupInParty = codes.some((c, i) => codes.indexOf(c) !== i);
    if (hasDupInParty) {
      toast("Duplicate item codes found within this party!", "error");
      return;
    }

    // Check for duplicate item codes in other parties (excluding original item codes of this party)
    const originalCodes = new Set(editEntry.items.map((it) => cleanStr(it._originalItemCode || "")));
    const otherData = data.filter((d) => d.partyCode !== editingPartyCode);
    const otherCodes = new Set(otherData.map((d) => cleanStr(d.itemCode)));

    const conflicting = editEntry.items.find(
      (it) => !originalCodes.has(cleanStr(it.itemCode)) && otherCodes.has(cleanStr(it.itemCode)),
    );
    if (conflicting) {
      toast(`Item Code "${conflicting.itemCode}" already exists in another party!`, "error");
      return;
    }
    ...
    saveToStorage([...otherData, ...updatedRows]);
  ...
  const processFile = (file) => {
    ...
    reader.onload = (evt) => {
      ...
      for (let i = 0; i < Math.min(rawData.length, 20); i++) {
        if (rawData[i].some((c) => cleanStr(c).includes("partycode"))) {
          hIdx = i;
          break;
        }
      }
      if (hIdx === -1) {
        toast("Missing required 'Party Code' column in the Excel file", "error");
        setUploadStep(1);
        return;
      }
      const hr = rawData[hIdx];
      const gci = (kws) =>
        hr.findIndex((c) => {
          const s = String(c).toLowerCase().replace(/[^a-z]/g, "");
          return kws.every((k) => s.includes(k));
        });
      const idxPC = gci(["party", "code"]),
        idxPD = gci(["party", "desc"]) !== -1 ? gci(["party", "desc"]) : gci(["party", "name"]);
      let idxPT = gci(["party", "type"]);
      if (idxPT === -1) idxPT = gci(["type"]);
      const idxPS = gci(["product", "segment"]),
        idxIC = gci(["item", "code"]),
        idxID = gci(["item", "desc"]),
        idxWP = gci(["warranty", "period"]);
      const idxSt = gci(["state"]),
        idxDC = gci(["district"]) !== -1 ? gci(["district"]) : gci(["city"]);
      const processed = rawData.slice(hIdx + 1).map((row) => {
        ...
        return {
          ...
          productSegment: idxPS !== -1 ? row[idxPS] : "",
          itemCode: idxIC !== -1 ? row[idxIC] : "",
          itemDescription: idxID !== -1 ? row[idxID] : "",
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirmed) {
      ...
      toast(`${count} items deleted successfully`);
    }
  };
              ...
              <div className="px-[2vw] py-[1.2vw] flex-1 flex flex-col justify-center min-h-[25vw]">
                ...
                {uploadStep === 4 && (
                  <div className="flex flex-col items-center justify-center h-full w-full">
                    <h3 className="text-[1.6vw] font-bold text-gray-800 mb-[0.5vw]">Import Successful!</h3>
                    <p className="text-[0.9vw] text-gray-500 mb-[2.5vw]">Your customer database has been successfully updated.</p>
                    <div className="grid grid-cols-3 gap-[1.5vw] w-full max-w-[35vw] mb-[3vw]">
                      <div className="bg-green-50 border border-green-100 rounded-[0.6vw] p-[1.2vw] flex flex-col items-center">
                        <span className="text-[1.8vw] font-bold text-green-700">{validationResult.valid}</span>
                        <span className="text-[0.75vw] font-semibold text-green-600 uppercase tracking-wide">Records Added</span>
                      </div>
                      <div className="bg-blue-50 border border-blue-100 rounded-[0.6vw] p-[1.2vw] flex flex-col items-center">
                        <span className="text-[1.8vw] font-bold text-blue-600">{validationResult.duplicates}</span>
                        <span className="text-[0.75vw] font-semibold text-blue-500 uppercase tracking-wide">Duplicates Skipped</span>
                      </div>
                      <div className="bg-red-50 border border-red-100 rounded-[0.6vw] p-[1.2vw] flex flex-col items-center">
                        ...
                        <span className="text-[0.75vw] font-semibold text-red-500 uppercase tracking-wide">Failed Rows</span>
                      </div>
                    </div>
                    <button
                      onClick={resetUpload}
