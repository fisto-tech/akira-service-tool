...
import {
  ...
  AlertTriangle, Info, ChevronRight, Shield,
} from "lucide-react";
...
const RefInput = ({ label, value, icon: Icon, span = 1 }) => (
  <div className={`flex flex-col gap-[0.25vw] ${span === 2 ? "col-span-2" : span === 3 ? "col-span-3" : span === 4 ? "col-span-4" : ""}`}>
    ...
    <div className="bg-white border border-gray-300 rounded-[0.4vw] py-[0.45vw] px-[0.6vw] text-[0.8vw] text-gray-900 ">
      {value || "—"}
    </div>
  </div>
);
...
const ServiceResponseModal = ({ entry, product, employees, onSave, onClose }) => {
          ...
          <div className="bg-white rounded-[0.6vw] border border-gray-300 shadow-sm overflow-hidden">
            ...
            <div className="p-[1vw] space-y-[1.2vw]">
              ...
              <div className="grid grid-cols-4 gap-[1vw]">
                ...
                <div className="col-span-3 flex flex-col gap-[0.3vw]">
                  <label className="text-[0.8vw] font-semibold text-black">
                    Problem Description <span className="text-red-500">*</span>
                  </label>
                  <textarea rows="1" value={formData.problemDescription} onChange={e => sp("problemDescription", e.target.value)}
...
export default function ServiceMaterialInwardResponse({ currentUser: propUser }) {
  ...
  useEffect(() => { reload(); const iv = setInterval(reload, 3000); return () => clearInterval(iv); }, []);
  ...
  const myItems = useMemo(() => {
    ...
    entries.forEach(e => {
      e.products?.forEach(p => {
        if (p.assignedTo === currentUser?.userId) {
    ...
    return res;
  }, [entries, currentUser, filterStatus]);

  // Update handler
  const updateEntry = (id, productId, report) => {
    const all = lsLoad(INWARD_KEY, []);
    const updated = all.map(e => {
      if (e.id === id) {
        return {
          ...e,
          products: e.products.map(p => p._pid === productId ? { ...p, report, status: report.status, remark: report.currentRemark } : p)
        };
      }
      return e;
    });
  ...
  const columns = [
    ...
    { key: "product",  label: "Product",     width: "w-[14vw]",  align: "text-left" },
      ...
      <div className="flex items-center justify-between mb-[1vw]">
        <div className="flex items-center gap-[0.6vw]">
          ...
          <div>
            ...
            <p className="text-[0.68vw] text-gray-500 font-medium italic">Service material inward items assigned to you</p>
          </div>
        </div>
        {myItems.length >= 0 && (
          <StatsBar 
            items={entries.flatMap(e => e.products?.filter(p => p.assignedTo === currentUser?.userId).map(p => ({ product: p })) || [])} 
                ...
                {myItems.map(({ entry, product }, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-[0.8vw] py-[0.7vw] border border-gray-300">
                        <div className="flex items-center gap-[0.5vw]">
                          ...
                          <div className="min-w-0">
                            <div className="text-[0.75vw] font-semibold text-gray-800 truncate">{entry.customerName}</div>
                      ...
                      <td className="px-[0.8vw] py-[0.7vw] border border-gray-300">
                        <div className="min-w-0">
                          <div className="text-[0.75vw] font-semibold text-gray-800 truncate" title={product.productDescription}>
